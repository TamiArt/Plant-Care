...
import { Bell, CheckSquare, Droplets, FlaskConical, Home, Lightbulb, List, Pencil, Plus, Square, StickyNote, Trash2, Trees, Wind, X } from "lucide-react";
    ...
    <div className="fixed inset-0 z-40 flex flex-col">
      ...
      >
          ...
          <div className="px-5 pt-3">
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="flex gap-2">
                <button onClick={onEdit} aria-label="Редактировать растение" className="w-9 h-9 rounded-full bg-secondary flex items-center justify-center flex-shrink-0"><Pencil size={15} className="text-primary" /></button>
                <button onClick={onRemove} aria-label="Удалить растение" className="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center flex-shrink-0"><Trash2 size={15} className="text-red-400" /></button>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              {fertOverdue && <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 flex items-center gap-1"><FlaskConical size={10} /> Удобрить</span>}
              {mistingEnabled && (
              ...
              {up.supplementalLight && (
                <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 flex items-center gap-1">
                  <Lightbulb size={10} /> {formatSupplementalLight(up.supplementalLight)}
                </span>
              )}
            </div>
              ...
              <div className="space-y-4 pb-4">
                ...
                {up.supplementalLight && (
                  <div className="bg-card border border-border rounded-2xl p-4">
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1"><Lightbulb size={11} /> Доп. освещение</p>
                    <p className="mt-2 text-sm font-semibold text-foreground">Ежедневно {formatSupplementalLight(up.supplementalLight)}</p>
                  </div>
                )}
                ...
                {mistingEnabled && (
                  <div className="bg-card border border-border rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1"><Wind size={11} /> Опрыскивание</p>
                ...
                <div className="bg-card border border-border rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1"><FlaskConical size={11} /> Удобрения</p>
                ...
                {up.customDescription && <div className="bg-secondary rounded-2xl p-4"><p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Описание</p><p className="text-sm text-foreground leading-relaxed">{up.customDescription}</p></div>}
              </div>
            )}
            ...
            {activeTab === "notes" && (
              <div className="space-y-3 pb-4">
                <div className="bg-secondary rounded-2xl p-3">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Новая заметка</p>
                  <div className="mb-2 flex gap-2">
                    <button type="button" onClick={() => addNotePrefix("- ")} className="flex items-center gap-1.5 rounded-lg bg-card px-2.5 py-1.5 text-[11px] font-medium text-foreground border border-border"><List size={13} /> Маркер</button>
                    <button type="button" onClick={() => addNotePrefix("[ ] ")} className="flex items-center gap-1.5 rounded-lg bg-card px-2.5 py-1.5 text-[11px] font-medium text-foreground border border-border"><CheckSquare size={13} /> Чекбокс</button>
                  </div>
                  <textarea ref={noteInputRef} value={noteText} onChange={e => setNoteText(e.target.value)} placeholder={"Например:\n- Пересадить весной\n[ ] Купить горшок\nОтцвела 15 июня"} rows={4}
                  ...
                  <button onClick={() => { if (noteText.trim()) { onAddNote(noteText.trim()); setNoteText(""); } }} disabled={!noteText.trim()}
            ...
            {activeTab === "reminders" && (
              <div className="space-y-3 pb-4">
                <div className="bg-secondary rounded-2xl p-3">
                  ...
                  <input value={reminderTitle} onChange={e => setReminderTitle(e.target.value)} placeholder="Что нужно сделать?" className="w-full bg-card rounded-xl px-3 py-2.5 text-sm outline-none placeholder:text-muted-foreground border border-border mb-2" />
                  <input type="date" value={reminderDate} onChange={e => setReminderDate(e.target.value)} className="w-full bg-card rounded-xl px-3 py-2.5 text-sm outline-none border border-border mb-2" />
                  <button onClick={() => { if (reminderTitle.trim()) { onAddReminder(reminderTitle.trim(), reminderDate); setReminderTitle(""); setReminderDate(todayStr()); } }} disabled={!reminderTitle.trim()}
                  ...
                  [...up.reminders].sort((a, b) => a.date.localeCompare(b.date)).map(r => {
                    ...
                    return (
                      <div key={r.id} className={`bg-card border rounded-2xl p-3.5 flex items-start gap-3 ${r.done ? "border-border opacity-60" : isPast ? "border-red-200 bg-red-50/30" : "border-border"}`}>
                        <button onClick={() => onToggleReminder(r.id)} className="mt-0.5 flex-shrink-0">{r.done ? <CheckSquare size={18} className="text-primary" /> : <Square size={18} className="text-muted-foreground" />}</button>
                        <div className="flex-1 min-w-0"><p className={`text-sm font-medium leading-tight ${r.done ? "line-through text-muted-foreground" : "text-foreground"}`}>{r.title}</p><p className={`text-xs mt-0.5 ${isPast && !r.done ? "text-red-500" : "text-muted-foreground"}`}>{isPast && !r.done ? "⚠️ " : "📅 "}{formatDate(r.date)}</p></div>
                        <button onClick={() => onDeleteReminder(r.id)}><Trash2 size={13} className="text-muted-foreground hover:text-red-400 transition-colors" /></button>
                      </div>
                    );
                  })
                )}
        ...
        <div className="px-5 pb-6 pt-3 border-t border-border bg-background flex gap-3 flex-shrink-0">
          <button onClick={onWater} className="flex-1 bg-primary text-primary-foreground py-3.5 rounded-2xl font-semibold flex items-center justify-center gap-2 text-sm"><Droplets size={16} /> Полить</button>
          {mistingEnabled && (
            <button onClick={onMist} disabled={mistToday}
