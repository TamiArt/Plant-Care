...
function normalizeSyncPlant(value: unknown): SyncPlant {
  ...
  if (!isValidDateString(value.createdAt)) throw new Error(`У растения ${value.id} отсутствует корректный createdAt.`);
  if (!isValidDateString(value.updatedAt)) throw new Error(`У растения ${value.id} отсутствует корректный updatedAt.`);
  if (value.deletedAt !== null && value.deletedAt !== undefined && !isValidDateString(value.deletedAt)) {
    throw new Error(`У растения ${value.id} некорректный deletedAt.`);
  }

  const location = value.location === "outdoor" ? "outdoor" : "home";
  const photoIds = stringArray(value.photoIds).slice(-3);
  const legacyPhotoId = nullableString(value.photoId);
  ...
  return {
    ...
    photoIds: photoIds.length > 0 ? photoIds : legacyPhotoId ? [legacyPhotoId] : [],
    wateringInterval: positiveNumber(value.wateringInterval, 7),
    wateringHistory: stringArray(value.wateringHistory),
    mistingHistory: stringArray(value.mistingHistory),
    fertilizingInterval: positiveNumber(value.fertilizingInterval, 30),
    ...
    location,
    notes: unknownArray(value.notes),
    reminders: unknownArray(value.reminders),
...
function safeJsonObject(value: string | null): unknown | undefined {
...
function rowToPlant(row: PlantRow): SyncPlant {
  return {
    ...
    wateringHistory: safeStringArray(row.watering_history),
    mistingHistory: safeStringArray(row.misting_history),
    fertilizingInterval: row.fertilizing_interval,
    fertilizingHistory: safeStringArray(row.fertilizing_history),
    addedAt: row.added_at,
    location: row.location === "outdoor" ? "outdoor" : "home",
    notes: safeJsonArray(row.notes),
    reminders: safeJsonArray(row.reminders),
    externalTaxon: safeJsonObject(row.external_taxon),
...
async function requireUser(request: Request, env: SyncEnv): Promise<AuthenticatedUser | null> {
...
async function ensureAppUser(env: SyncEnv, user: AuthenticatedUser): Promise<void> {
  ...
  `).bind(user.id, user.email, now).run();
}

function createPlantUpsert(env: SyncEnv, userId: string, plant: SyncPlant): D1PreparedStatement {
  return env.DB.prepare(`
    INSERT INTO plants (
      id, user_id, catalog_id, custom_name, custom_latin_name,
      custom_description, custom_emoji, nickname, photo_id, photo_ids,
      watering_interval, watering_history, misting_history,
      fertilizing_interval, fertilizing_history, added_at, location,
      notes, reminders, external_taxon, created_at, updated_at, deleted_at
    ) VALUES (
      ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
    )
    ON CONFLICT(id) DO UPDATE SET
  ...
  `).bind(
    ...
    JSON.stringify(plant.mistingHistory),
...
async function getPlantsForUser(env: SyncEnv, userId: string): Promise<SyncPlant[]> {
...
export async function handleGetPlants(request: Request, env: SyncEnv): Promise<Response> {
  const user = await requireUser(request, env);
  if (!user) return json({ error: "Unauthorized" }, { status: 401 });

  await ensureAppUser(env, user);
  const plants = await getPlantsForUser(env, user.id);
  return json({ plants, syncedAt: new Date().toISOString() });
}

export async function handleSyncPlants(request: Request, env: SyncEnv): Promise<Response> {
  const user = await requireUser(request, env);
  if (!user) return json({ error: "Unauthorized" }, { status: 401 });

  let body: SyncRequestBody;
  try {
    body = await request.json<SyncRequestBody>();
  } catch {
    return json({ error: "Некорректный JSON." }, { status: 400 });
  }

  if (!Array.isArray(body.plants)) {
    return json({ error: "Поле plants должно быть массивом." }, { status: 400 });
  }

  let plants: SyncPlant[];
  try {
    plants = body.plants.map(normalizeSyncPlant);
  } catch (error) {
    return json({
      error: error instanceof Error ? error.message : "Некорректные данные растений.",
    }, { status: 400 });
  }

  await ensureAppUser(env, user);

  const remotePlants = await getPlantsForUser(env, user.id);
  const remoteById = new Map(remotePlants.map(plant => [plant.id, plant]));
  const serverNow = new Date().toISOString();

  const plantsToPersist = plants.map(plant => {
    const remote = remoteById.get(plant.id);
    return remote
      ? mergeCareHistoryPlant(remote, plant, serverNow)
      : plant;
  });

  if (plantsToPersist.length > 0) {
    const statements = plantsToPersist.map(plant =>
      createPlantUpsert(env, user.id, plant),
    );
    await env.DB.batch(statements);
  }

  const mergedPlants = await getPlantsForUser(env, user.id);
  return json({ plants: mergedPlants, syncedAt: new Date().toISOString() });
}
