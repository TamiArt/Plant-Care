 ...
 *
 * userId намеренно отсутствует:
 * владелец определяется только сервером
 * по Better Auth session cookie.
 */
 ...
 *
 * Better Auth хранит пользователя в своей
 * таблице user, а здесь мы создаём профиль,
 * на который ссылаются растения.
 */
...
export async function handleSyncPlants(
   ...
   * набор пользователя.
   *
   * Если на сервере версия была новее,
   * UPDATE выше её не перезапишет.
   */
  const mergedPlants =
    await getPlantsForUser(
      env,
